...
const HUNGAROCONTROL_AIS_URL = "https://ais.hungarocontrol.hu";
const AirspaceUsePlanMap = lazy(() =>
  import("@/notams/AirspaceUsePlanMap").then((module) => ({
    default: module.AirspaceUsePlanMap,
  })),
);
...
export function NotamPanel({ waypoints }: Props) {
  ...
  return (
    <section id="notams" className="space-y-4">
      ...
      <Card className="overflow-hidden">
        <CardHeader className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b border-border bg-panel-muted px-4 py-3">
          <CardTitle className="panel-heading">HungaroControl airspace use plan</CardTitle>
          <Button asChild variant="outline" size="sm">
            <a
              href={`${HUNGAROCONTROL_AIS_URL}/legterfelhasznalasi-terv`}
              target="_blank"
              rel="noreferrer"
            >
              <ExternalLink className="size-4" />
        ...
        <Suspense
          fallback={
            <div className="grid h-[560px] place-items-center bg-panel-muted font-mono text-xs uppercase tracking-[0.14em] text-muted-foreground">
              Loading airspace map
            </div>
          }
        >
          <AirspaceUsePlanMap />
