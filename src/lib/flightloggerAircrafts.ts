...
async function fetchAllAircraft(
  ...
  while (hasNextPage && page < MAX_PAGES) {
    const variables = {
      after: cursor,
      first: pageSize,
      callSigns: options.callSigns.length ? options.callSigns : undefined,
    };

    let data: AircraftConnection;

    try {
      data = await requestFlightLogger<AircraftConnection>(
        endpoint,
        token,
        variables,
        query,
      ...
      if (isComplexityError(message)) {
         ...
         */
        if (query !== BASIC_AIRCRAFT_QUERY && page === 0) {
      ...
      if (
        ...
        return fetchAircraftFromBookings(endpoint, token, options);
      }

      throw error;
    }

    const nodes = data.aircraft?.nodes ?? [];

    allNodes.push(...nodes);

    endCursor = data.aircraft?.pageInfo?.endCursor;

    hasNextPage = Boolean(data.aircraft?.pageInfo?.hasNextPage && endCursor);
  ...
  const normalized = allNodes
    ...
    .filter((item): item is FlightLoggerAircraft => item !== null);

  const aircraft = filterAircraft(normalized, options);
...
async function fetchAircraftFromBookings(
  ...
  const seen = new Map<string, FlightLoggerAircraft>();
  ...
  const to = new Date(Date.UTC(now.getUTCFullYear() + 2, 0, 1));

  while (hasNextPage && page < MAX_PAGES) {
    const data = await requestFlightLogger<BookingConnection>(
      endpoint,
      token,
      {
    ...
    for (const edge of edges) {
      collectAircraft(edge.node, seen);
    }
...
async function requestFlightLogger<T>(
  ...
  if (!response.ok || payload?.errors?.length) {
     ...
     */
    if (isComplexityError(resolvedMessage)) {
...
function isComplexityError(message: string) {
...
function collectAircraft(
  value: unknown,
  seen: Map<string, FlightLoggerAircraft>,
) {
  if (Array.isArray(value)) {
    for (const item of value) {
      collectAircraft(item, seen);
    }
    return;
  }
  ...
  const aircraft = normalizeAircraft(value.aircraft);

  if (aircraft && !seen.has(aircraft.id)) {
    seen.set(aircraft.id, aircraft);
  }
...
function normalizeMaintenancePart(input: unknown) {
  const record = recordValue(input);
...
function filterAircraft(
  ...
  const wantedCallSigns = new Set(
    options.callSigns
      .map((value) => value.trim().toUpperCase())
      .filter(Boolean),
  );

  const wantedClass = options.aircraftClass?.trim().toUpperCase() || null;

  return aircraft.filter((item) => {
    if (
      wantedCallSigns.size > 0 &&
      !wantedCallSigns.has(item.callSign.trim().toUpperCase())
    ) {
      return false;
    }
    ...
    if (
      wantedClass &&
      (item.aircraftClass ?? "").trim().toUpperCase() !== wantedClass
    ) {
      return false;
    }
